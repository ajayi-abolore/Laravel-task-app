                        ...
                        <div  class="col-fixed " >
                            <template v-if="auth.canView('permissions/add')">
                                <router-link :to="`/permissions/add`">
                                    <Button label="Add New Permission" icon="pi pi-plus" type="button" class="p-button w-full bg-primary "  />
                                </router-link>
                            </template>
                        </div>
                                ...
                                <DataTable :lazy="true"   :loading="loading"    v-model:selection="selectedItems"
                                    ...
                                    <Column selectionMode="multiple" headerStyle="width: 2rem" />
                                        ...
                                        <Column  field="role_id" header="Role Id" >
                                            <template #body="{ data, index }">
                                                <Button class="p-button-text" icon="pi pi-eye" label="Roles" v-if="data.role_id" @click="app.openPageDialog({ page: 'roles/view', url: `/roles/view/${data.role_id}` , closeBtn: true })" />
                                            </template>
                                        </Column>
