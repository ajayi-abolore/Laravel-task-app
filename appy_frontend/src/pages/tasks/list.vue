            ...
            <section class="page-section mb-3" >
                <div class="container-fluid">
                    <div class="grid justify-content-between align-items-center">
                        ...
                        <div  class="col-fixed " >
                            <template v-if="auth.canView('tasks/add')">
                                <router-link :to="`/tasks/add`">
                                    <Button label="Add New Task" icon="pi pi-plus" type="button" class="p-button w-full bg-primary "  />
                                </router-link>
                            </template>
                        </div>
                    ...
                    <div  class="col comp-grid" >
                        ...
                        <div >
                            <template v-if="showBreadcrumbs && $route.query.tag && !isSubPage">
                                <Breadcrumb :home="{icon: 'pi pi-home', to: '/tasks'}" :model="pageBreadCrumb" />
                            </template>
                                    ...
                                    <Column selectionMode="multiple" headerStyle="width: 2rem" />
                                        <Column  field="id" header="Id" >
                                            <template #body="{ data, index }">
                                                <router-link :to="`/tasks/view/${data.id}`">
                                                    {{ data.id }}
                                                </router-link>
                                            </template>
                                        </Column>
                                        ...
                                        <Column  field="due_time" header="Due Time" >
                                            <template #body="{ data, index }">
                                                {{ data.due_time }}
                                            </template>
                                        </Column>
                                        <Column  field="assign_to" header="Assign To" >
                                            <template #body="{ data, index }">
                                                {{ data.assign_to }}
                                            </template>
                                        </Column>
                                ...
                                <template v-if="showFooter && pageReady">
                                    <div class="grid justify-content-between align-items-center">
                                        <div class="flex gap-2 flex-grow-0">
                                            <template v-if="auth.canView('tasks')">
...
<script setup>
	...
	function getActionMenuModel(data){
		...
		{
			label: "View",
			to: `/tasks/view/${data.id}`,
			icon: "pi pi-eye",
			visible: auth.canView('tasks/view')
		},
		{
			label: "Edit",
			to: `/tasks/edit/${data.id}`,
			icon: "pi pi-pencil",
